...
contour(grid.x, grid.y, grid.z, main=expression(k[N] == 1000), xlab=expression(theta[1]), 
        ylab=expression(theta[2]), xlim=lim.x, ylim=lim.y, asp=1)
points(ret[[1]], pch=".", cex=1.5)
contour(grid.x, grid.y, grid.z, main=expression(k[N] == 10000), xlab=expression(theta[1]), 
        ylab=expression(theta[2]), xlim=lim.x, ylim=lim.y, asp=1)
points(ret[[2]][sample(10000, size=1000, replace=TRUE), ], pch=".", cex=1.5)
contour(grid.x, grid.y, grid.z, main=expression(k[N] == 100000), xlab=expression(theta[1]), 
        ylab=expression(theta[2]), xlim=lim.x, ylim=lim.y, asp=1)
points(ret[[3]][sample(100000, size=1000, replace=TRUE), ], pch=".", cex=1.5)
contour(grid.x, grid.y, grid.z, main=expression(k[N] == 1000000), xlab=expression(theta[1]), 
        ylab=expression(theta[2]), xlim=lim.x, ylim=lim.y, asp=1)
